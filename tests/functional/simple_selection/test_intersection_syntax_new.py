...
def test_same_model_intersection(project, create_tables):

    results = run_dbt(["run", "--models", "users,users"])
    verify_selected_users(project, results)


def test_same_model_intersection_selectors(project, create_tables):

    results = run_dbt(["run", "--selector", "same_intersection"])
    verify_selected_users(project, results)


def test_tags_intersection(project, create_tables):

    results = run_dbt(["run", "--models", "tag:bi,tag:users"])
    verify_selected_users(project, results)


def test_tags_intersection_selectors(project, create_tables):

    results = run_dbt(["run", "--selector", "tags_intersection"])
    verify_selected_users(project, results)


def test_intersection_triple_descending(project, create_tables):

    results = run_dbt(["run", "--models", "*,tag:bi,tag:users"])
    verify_selected_users(project, results)


def test_intersection_triple_descending_schema(project, create_tables):

    results = run_dbt(["run", "--models", "*,tag:bi,tag:users"])
    verify_selected_users(project, results)


def test_intersection_triple_descending_schema_selectors(project, create_tables):

    results = run_dbt(["run", "--selector", "triple_descending"])
    verify_selected_users(project, results)


def test_intersection_triple_ascending(project, create_tables):

    results = run_dbt(["run", "--models", "tag:users,tag:bi,*"])
    verify_selected_users(project, results)


def test_intersection_triple_ascending_schema_selectors(project, create_tables):
...
def test_intersection_with_exclusion(project, create_tables):
...
def test_intersection_with_exclusion_selectors(project, create_tables):

    results = run_dbt(["run", "--selector", "intersection_with_exclusion"])
    verify_selected_users_and_rollup(project, results)


def test_intersection_exclude_intersection(project, create_tables):

    results = run_dbt(["run", "--models", "tag:bi,@users", "--exclude", "tag:bi,users_rollup+"])
    verify_selected_users(project, results)


def test_intersection_exclude_intersection_selectors(project, create_tables):

    results = run_dbt(["run", "--selector", "intersection_exclude_intersection"])
    verify_selected_users(project, results)


def test_intersection_exclude_intersection_lack(project, create_tables):

    results = run_dbt(["run", "--models", "tag:bi,@users", "--exclude", "@emails,@emails_alt"])
    verify_selected_users_and_rollup(project, results)


def test_intersection_exclude_intersection_lack_selector(project, create_tables):

    results = run_dbt(["run", "--selector", "intersection_exclude_intersection_lack"])
    verify_selected_users_and_rollup(project, results)


def test_intersection_exclude_triple_intersection(project, create_tables):

    results = run_dbt(["run", "--models", "tag:bi,@users", "--exclude", "*,tag:bi,users_rollup"])
    verify_selected_users(project, results)


def test_intersection_concat(project, create_tables):
...
def test_intersection_concat_intersection(project, create_tables):
...
def test_intersection_concat_exclude(project, create_tables):
...
def test_intersection_concat_exclude_concat(project, create_tables):
...
def test_intersection_concat_exclude_intersection_concat(project, create_tables):
