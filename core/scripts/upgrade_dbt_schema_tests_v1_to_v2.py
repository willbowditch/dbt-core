...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "if set, apply changes instead of just logging about found "
            "schema.yml files"
        ),
    )
    parser.add_argument(
...
def handle(parsed):
    ...
    for model_dir in model_dirs:
        ...
        convert_project(
            search_path, parsed.backup, parsed.apply, parsed.extra_complex_tests
        )
    if not parsed.apply:
        print(
...
def main(args=None):
    ...
    try:
        ...
        LOGGER.info(
            "successfully converted files in {}".format(parsed.search_directory)
        )
...
for cls in (
    CustomSortedRootSchema,
    CustomSortedModelsSchema,
    CustomSortedColumnsSchema,
):
...
if __name__ == "__main__":
    ...
    class TestConvert(unittest.TestCase):
        ...
        def test_convert(self):
            ...
            output_schema = convert_schema(
                input_schema, {"known_complex_custom": "field"}
            )
