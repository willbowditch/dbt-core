...
REQUIRED_COMPILED_NODE_KEYS = frozenset(
    ...
    | {"compiled", "extra_ctes_injected", "extra_ctes", "compiled_sql", "relation_name"}
)
...
class ManifestTest(unittest.TestCase):
    ...
    def test__build_flat_graph(self):
        ...
        self.assertEqual(
            set(flat_graph), set(["exposures", "nodes", "sources", "metrics"])
        )
...
class MixedManifestTest(unittest.TestCase):
    ...
    def test__build_flat_graph(self):
        ...
        flat_nodes = flat_graph["nodes"]
        self.assertEqual(
            set(flat_graph), set(["exposures", "metrics", "nodes", "sources"])
        )
        self.assertEqual(set(flat_nodes), set(self.nested_nodes))
        compiled_count = 0
        for node in flat_nodes.values():
...
generate_name_parameter_sets = [
    ...
    FindMacroSpec(
        macros=[
            MockGenerateMacro("root"),
            MockGenerateMacro("dep"),
            MockGenerateMacro("dbt"),
        ],
        expected="root",
    ),
]
...
FindMaterializationSpec = namedtuple(
    "FindMaterializationSpec", "macros,adapter_type,expected"
)
...
def _docs_parameter_sets():
    ...
    sets.extend(
        [
            # exists in other
            FindDocSpec(
                docs=[MockDocumentation("dep", "my_doc")], package="root", expected=None
            ),
            FindDocSpec(
