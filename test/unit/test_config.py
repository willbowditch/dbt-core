...
class TestProfile(BaseConfigTest):
    ...
    def test_invalid_env_vars(self):
        ...
        self.assertIn(
            "Could not convert value 'hello' into type 'number'", str(exc.exception)
        )
...
class TestProfileFile(BaseFileTest):
    ...
    def test_cli_and_env_vars(self):
        ...
        renderer = dbt.config.renderer.ProfileRenderer(
            {"cli_value_host": "cli-postgres-host"}
        )
        with mock.patch.dict(os.environ, self.env_override):
    ...
    def test_profile_with_empty_profile_data(self):
        ...
        self.assertIn(
            "Profile empty_profile_data in profiles.yml is empty", str(exc.exception)
        )
...
class TestProject(BaseConfigTest):
    ...
    def test_defaults(self):
        ...
        self.assertEqual(
            project.docs_paths, ["models", "seeds", "snapshots", "analyses", "macros"]
        )
    ...
    def test_implicit_overrides(self):
        ...
        self.assertEqual(
            project.docs_paths,
            ["other-models", "seeds", "snapshots", "analyses", "macros"],
        )
...
class TestRunOperationTask(BaseFileTest):
    ...
    def test_run_operation_task(self):
        ...
        self.assertEqual(
            os.path.realpath(os.getcwd()), os.path.realpath(self.project_dir)
        )
...
class TestVariableProjectFile(BaseFileTest):
    ...
    def test_cli_and_env_vars(self):
        renderer = dbt.config.renderer.DbtProjectYamlRenderer(
...
class TestRuntimeConfig(BaseConfigTest):
    ...
    def test_supported_version(self):
        ...
        self.assertEqual(
            set(x.to_version_string() for x in conf.dbt_version), {">0.0.0"}
        )
    ...
    def test_unsupported_version_no_check(self):
        ...
        conf = self.from_parts()
        self.assertEqual(
            set(x.to_version_string() for x in conf.dbt_version), {">99999.0.0"}
        )
    ...
    def test_supported_version_range(self):
        ...
        conf = self.from_parts()
        self.assertEqual(
            set(x.to_version_string() for x in conf.dbt_version),
            {">0.0.0", "<=99999.0.0"},
        )
    ...
    def test_impossible_version_range(self):
        ...
        raised = self.from_parts(dbt.exceptions.DbtProjectError)
        self.assertIn(
            "The package version requirement can never be satisfied",
...
class TestRuntimeConfigWithConfigs(BaseConfigTest):
    ...
    def test__warn_for_unused_resource_config_paths_disabled(self):
        project = self.from_parts()
        unused = project.get_unused_resource_config_paths(
            self.used, frozenset([("my_test_project", "baz")])
        )
...
class TestRuntimeConfigFiles(BaseFileTest):
    ...
    def test_from_args(self):
        ...
        self.assertEqual(
            config.docs_paths, ["models", "seeds", "snapshots", "analyses", "macros"]
        )
        ...
        self.assertEqual(
            config.quoting, {"database": True, "identifier": True, "schema": True}
        )
...
class TestVariableRuntimeConfigFiles(BaseFileTest):
    ...
    def test_cli_and_env_vars(self):
        self.args.target = "cli-and-env-vars"
        self.args.vars = (
            '{"cli_value_host": "cli-postgres-host", "cli_version": "0.1.2"}'
        )
        ...
        self.assertEqual(
            config.models["foo"]["post-hook"], "{{ env_var('env_value_profile') }}"
        )
        self.assertEqual(config.models["bar"]["materialized"], "default")  # rendered!
        self.assertEqual(
            config.seeds["foo"]["post-hook"], "{{ env_var('env_value_profile') }}"
        )
