...
class SchemaParserSourceTest(SchemaParserTest):
    ...
    def test__read_basic_source_tests(self):
        ...
        self.assertEqual(source_values[0].table.name, "my_table")
        self.assertEqual(
            source_values[0].table.description, "A description of my table"
        )
...
class SchemaParserModelsTest(SchemaParserTest):
    ...
    def test__parse_basic_model_tests(self):
        ...
        self.assertEqual(tests[0].fqn, ["snowplow", tests[0].name])
        self.assertEqual(
            tests[0].unique_id.split("."),
            ["test", "snowplow", tests[0].name, "9d4814efde"],
        )
        ...
        self.assertEqual(
            tests[1].unique_id.split("."),
            ["test", "snowplow", tests[1].name, "13958f62f7"],
        )
        ...
        self.assertEqual(
            tests[2].unique_id.split("."),
            ["test", "snowplow", tests[2].name, "2f61818750"],
        )
        ...
        self.assertEqual(
            self.parser.manifest.files[file_id].node_patches, ["model.root.my_model"]
        )
...
class SnapshotParserTest(BaseParserTest):
    ...
    def test_single_block(self):
        ...
        self.assertIn(file_id, self.parser.manifest.files)
        self.assertEqual(
            self.parser.manifest.files[file_id].nodes, ["snapshot.snowplow.foo"]
        )
