...
@pytest.mark.parametrize(
    "value,text_expectation,native_expectation", jinja_tests, ids=expected_id
)
...
class TestBlockLexer(unittest.TestCase):
    ...
    def test_complex_file(self):
        ...
        self.assertEqual(blocks[0].block_name, "foo")
        self.assertEqual(
            blocks[0].full_block, "{% mytype foo %} some stuff {% endmytype %}"
        )
    ...
    def test_macro_with_crazy_args(self):
        ...
        blocks = extract_toplevel_blocks(
            body, allowed_blocks={"macro"}, collect_raw_data=False
        )
        self.assertEqual(len(blocks), 1)
    ...
    def test_embedded_self_closing_comment_block(self):
        ...
        blocks = extract_toplevel_blocks(
            body, allowed_blocks={"myblock"}, collect_raw_data=False
        )
        self.assertEqual(len(blocks), 1)
        self.assertEqual(blocks[0].full_block, body)
    ...
    def test_crazy_set_statement(self):
        ...
        self.assertEqual(len(blocks), 1)
        self.assertEqual(
            blocks[0].full_block, "{% otherblock bar %}x{% endotherblock %}"
        )
    ...
    def test_docs_block(self):
        ...
        blocks = extract_toplevel_blocks(
            body, allowed_blocks={"docs"}, collect_raw_data=False
        )
        self.assertEqual(len(blocks), 2)
    ...
    def test_unclosed_model_quotes(self):
        ...
        blocks = extract_toplevel_blocks(
            body, allowed_blocks={"model"}, collect_raw_data=False
        )
        self.assertEqual(len(blocks), 1)
        self.assertEqual(blocks[0].block_type_name, "model")
        self.assertEqual(
            blocks[0].contents, 'select * from "something"."something_else'
        )
