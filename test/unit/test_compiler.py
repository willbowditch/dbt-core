...
class CompilerTest(unittest.TestCase):
    def assertEqualIgnoreWhitespace(self, a, b):
        self.assertEqual("".join(a.split()), "".join(b.split()))
    ...
    def setUp(self):
        ...
        self.mock_generate_runtime_model_context = (
            self._generate_runtime_model_context_patch.start()
        )
        ...
        self.mock_generate_runtime_model_context.side_effect = (
            mock_generate_runtime_model_context_meth
        )
    ...
    def test__prepend_ctes__already_has_cte(self):
        ...
        self.assertEqual(manifest.nodes["model.root.ephemeral"].extra_ctes_injected, True)
    ...
    def test__prepend_ctes__no_ctes(self):
        manifest = Manifest(
            ...
            nodes={
                "model.root.view": ParsedModelNode(
                    ...
                    original_file_path="view.sql",
                    raw_sql=(
                        "with cte as (select * from something_else) " "select * from source_table"
                    ),
                    checksum=FileHash.from_contents(""),
                ),
        ...
        self.assertEqual(result, manifest.nodes.get("model.root.view"))
        ...
        self.assertEqual(result, manifest.nodes.get("model.root.view_no_cte"))
        self.assertTrue(result.extra_ctes_injected)
        self.assertEqualIgnoreWhitespace(result.compiled_sql, "select * from source_table")
    ...
    def test__prepend_ctes__cte_not_compiled(self):
        ...
        manifest = Manifest(
            ...
            nodes={
                "model.root.view": CompiledModelNode(
                    ...
                    extra_ctes=[
                        InjectedCTE(id="model.root.ephemeral", sql="select * from source_table")
                    ],
        ...
        with patch.object(compiler, "_compile_node") as compile_node:
            ...
            result, _ = compiler._recursively_prepend_ctes(
                manifest.nodes["model.root.view"], manifest, {}
            )
