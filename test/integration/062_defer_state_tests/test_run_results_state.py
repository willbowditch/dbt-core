...
class TestRunResultsState(DBTIntegrationTest):
    ...
    def test_postgres_seed_run_results_state(self):
        ...
        results = self.run_dbt(
            ["ls", "--select", "result:success", "--state", "./state"]
        )
        assert len(results) == 1
        assert results[0] == "test.seed"
        ...
        results = self.run_dbt(
            ["ls", "--select", "result:error+", "--state", "./state"]
        )
        assert len(results) == 7
        assert set(results) == {
            "test.seed",
    ...
    def test_postgres_build_run_results_state(self):
        results = self.run_dbt(
            ["build", "--select", "result:error", "--state", "./state"]
        )
        ...
        results = self.run_dbt(
            ["build", "--select", "result:fail", "--state", "./state"],
            expect_pass=False,
        )
        assert len(results) == 1
        ...
        results = self.run_dbt(
            ["build", "--select", "result:warn+", "--state", "./state"],
            expect_pass=True,
        )
    ...
    def test_postgres_run_run_results_state(self):
        results = self.run_dbt(
            ["run", "--select", "result:success", "--state", "./state"],
            expect_pass=True,
        )
        assert len(results) == 2
        ...
        results = self.run_dbt(
            ["run", "--select", "result:error", "--state", "./state"]
        )
        assert len(results) == 0
        ...
        results = self.run_dbt(
            ["run", "--select", "result:skipped", "--state", "./state"],
            expect_pass=True,
        )
        assert len(results) == 1
    ...
    def test_postgres_test_run_results_state(self):
        ...
        results = self.run_dbt(
            ["test", "--select", "result:fail+", "--state", "./state"],
            expect_pass=False,
        )
        assert len(results) == 1
        assert results[0].node.name == "unique_view_model_id"
    ...
    def test_postgres_concurrent_selectors_test_run_results_state(self):
        ...
        with open("./models/view_model.sql", "w") as f:
            f.write(
                "select 1 as id union all select 1 as id union all select null as id"
            )
