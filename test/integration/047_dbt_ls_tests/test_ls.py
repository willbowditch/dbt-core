...
class TestStrictUndefined(DBTIntegrationTest):
    ...
    def expect_all_output(self):
        ...
        results = self.run_dbt_ls(
            ["--resource-type", "all", "--select", "*", "source:*"]
        )
        ...
        results = self.run_dbt_ls(
            ["--resource-type", "default", "--select", "*", "source:*"]
        )
        self.assertEqual(set(results), expected_default)
    ...
    def expect_select(self):
        ...
        self.assertEqual(
            set(results), {"test.not_null_outer_id", "test.unique_outer_id"}
        )

        self.run_dbt_ls(
            ["--resource-type", "test", "--select", "inner"], expect_pass=True
        )
        ...
        self.assertEqual(
            set(results), {"test.ephemeral", "test.outer", "test.incremental"}
        )

        results = self.run_dbt_ls(
            ["--select", "config.incremental_strategy:delete+insert"]
        )
        self.assertEqual(set(results), {"test.incremental"})

        self.run_dbt_ls(
            ["--select", "config.incremental_strategy:insert_overwrite"],
            expect_pass=True,
        )
