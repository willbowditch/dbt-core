...
class TestSimpleDependency(BaseDependencyTest):
    ...
    def test_postgres_local_dependency(self):
        ...
        base_schema_nodes = [
            r.node for r in results if r.node.schema == self.base_schema()
        ]
        ...
        self.assertTablesEqual(
            "dep_source_model", "seed", self.base_schema(), self.base_schema()
        )
        ...
        cli_vars = json.dumps(
            {
                ...
                "test_create_second_table": '"{}"."hook_test_2"'.format(
                    self.unique_schema()
                ),
            }
        )
