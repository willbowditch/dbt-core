...
class BaseDependencyTest(DBTIntegrationTest):
    ...
    def setUp(self):
        super().setUp()
        self._created_schemas.add(self._get_schema_fqn(self.default_database, self.base_schema()))
...
class TestSimpleDependency(BaseDependencyTest):
    ...
    def test_postgres_local_dependency(self):
        ...
        self.assertEqual(
            {r.node.schema for r in results}, {self.base_schema(), self.configured_schema()}
        )

        base_schema_nodes = [r.node for r in results if r.node.schema == self.base_schema()]
        self.assertEqual(len(base_schema_nodes), 4)
        self.assertTablesEqual(
            "source_override_model", "seed", self.base_schema(), self.base_schema()
        )
