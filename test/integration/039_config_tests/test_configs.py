...
class TestTargetConfigs(DBTIntegrationTest):
    ...
    def setUp(self):
        super().setUp()
        self.init_targets = [
            d for d in os.listdir(".") if os.path.isdir(d) and d.startswith("target_")
        ]
    ...
    def new_dirs(self):
        for d in os.listdir("."):
            if (
                os.path.isdir(d)
                and d not in self.init_targets
                and d.startswith("target_")
            ):
                yield d
...
class TestConfigIndivTests(DBTIntegrationTest):
    ...
    def test_postgres_configuring_individual_tests(self):
        ...
        self.assertEqual(
            len(self.run_dbt(["test", "--vars", '{"enabled_direct": False}'])), 3
        )
