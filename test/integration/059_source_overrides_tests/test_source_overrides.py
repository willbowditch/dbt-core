...
class TestSourceOverrides(DBTIntegrationTest):
    ...
    def test_postgres_source_overrides(self):
        ...
        results = self.run_dbt(["source", "snapshot-freshness"], expect_pass=False)
        ...
        results = self.run_dbt(["source", "snapshot-freshness"], expect_pass=True)
        self.assertEqual(len(results), 1)
...
class TestSourceDuplicateOverrides(DBTIntegrationTest):
    ...
    def test_postgres_source_duplicate_overrides(self):
        ...
        self.assertIn(
            "dbt found two schema.yml entries for the same source named", str(exc.exception)
        )
