...
class TestSimpleSnapshotFileSelects(DBTIntegrationTest):
    ...
    def project_config(self):
        return {
            "config-version": 2,
            "seed-paths": ["seeds"],
            "snapshot-paths": ["snapshots-select", "snapshots-pg"],
            "macro-paths": ["macros"],
        }
    ...
    def run_snapshot(self):
        return self.run_dbt(
            [
                "snapshot",
                "--vars",
                '{{"target_schema": {}}}'.format(self.target_schema()),
            ]
        )
...
class TestBadSnapshot(DBTIntegrationTest):
    ...
    def test__postgres__invalid(self):
        ...
        self.assertIn(
            "Snapshots must be configured with a 'strategy'", str(exc.exception)
        )
...
class TestUpdatedAtCheckCols(TestCheckCols):
    def _assertTablesEqualSql(self, relation_a, relation_b, columns=None):
        ...
        for result in revived_records:
            ...
            self.assertEqual(
                result[1].replace(tzinfo=pytz.UTC), result[2].replace(tzinfo=pytz.UTC)
            )
...
class TestSnapshotHardDelete(DBTIntegrationTest):
    ...
    def _snapshot(self):
        ...
        results = self.run_dbt(
            ["snapshot", "--vars", "{invalidate_hard_deletes: true}"]
        )
        self.assertEqual(len(results), self.NUM_SNAPSHOT_MODELS)
    ...
    def _delete_records(self):
        ...
        self.run_sql(
            "delete from {}.{}.seed where id >= 10;".format(
                database, self.unique_schema()
            )
        )
    ...
    def _snapshot_and_assert_invalidated(self):
        ...
        for result in snapshotted[10:]:
            ...
            self.assertGreaterEqual(
                result[-1].replace(tzinfo=pytz.UTC), self._invalidated_snapshot_datetime
            )
    ...
    def _snapshot_and_assert_revived(self):
        ...
        for result in revived_records:
            ...
            self.assertGreaterEqual(
                ...
                self._invalidated_snapshot_datetime.replace(microsecond=0),
            )
