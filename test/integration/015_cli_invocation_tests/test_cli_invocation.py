...
class TestCLIInvocationWithProfilesDir(ModelCopyingIntegrationTest):
    ...
    def test_postgres_toplevel_dbt_run_with_profile_dir_arg(self):
        ...
        results = self.run_dbt(
            ["run", "--profiles-dir", "dbt-profile"], profiles_dir=False
        )
        self.assertEqual(len(results), 1)

        actual = self.run_sql(
            "select id from {}.model".format(self.custom_schema), fetch="one"
        )
        ...
        res = self.run_dbt(
            ["test", "--profiles-dir", "dbt-profile"], profiles_dir=False
        )
