...
class TestDocsGenerate(DBTIntegrationTest):
            ...
            "nodes": {
                "model.test.model": {
                    ...
                    "raw_sql": LineIndifferent(
                        _read_file(model_sql_path).rstrip("\r\n")
                    ),
                    "refs": [["seed"]],
                    "sources": [],
                ...
                "model.test.second_model": {
                    ...
                    "package_name": "test",
                    "raw_sql": LineIndifferent(
                        _read_file(second_model_sql_path).rstrip("\r\n")
                    ),
                    "refs": [["seed"]],
                    "sources": [],
                    "depends_on": {"nodes": ["seed.test.seed"], "macros": []},
    ...
    def verify_manifest(self, expected_manifest):
        ...
        for key in manifest_keys:
            if key == "macros":
                ...
                assert (
                    "adapter_type" in metadata
                    and metadata["adapter_type"] == self.adapter_type
                )
            else:
    ...
    def test__postgres__run_and_generate(self):
        ...
        self.verify_manifest(
            self.expected_seeded_manifest(model_database=self.default_database)
        )
