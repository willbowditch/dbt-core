...
class ModelTest(BasePPTest):
    ...
    def test_postgres_pp_models(self):
        ...
        self.assertEqual(
            model_one_node.patch_path, "test://" + normalize("models/schema.yml")
        )
...
class TestMacros(BasePPTest):
    ...
    def test_postgres_nested_macros(self):
        ...
        self.copy_file(
            "test-files/custom_schema_tests1.sql", "macros/custom_schema_tests.sql"
        )
        ...
        self.copy_file(
            "test-files/custom_schema_tests2.sql", "macros/custom_schema_tests.sql"
        )
        results = self.run_dbt(["--partial-parse", "test"], expect_pass=False)
        manifest = get_manifest()
...
class TestTests(BasePPTest):
    ...
    def test_postgres_pp_generic_tests(self):
        ...
        self.copy_file(
            "test-files/generic_test_edited.sql", "tests/generic/generic_test.sql"
        )
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 1)
        manifest = get_manifest()
