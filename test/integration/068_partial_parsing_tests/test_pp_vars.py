...
class EnvVarTest(BasePPTest):
    ...
    def test_postgres_env_vars_models(self):
        ...
        manifest = get_manifest()
        test_color_id = (
            "test.test.check_color_model_one_env_var_ENV_VAR_COLOR___fun.89638de387"
        )
        test_node = manifest.nodes[test_color_id]
        # kwarg was rendered but not changed (it will be rendered again when compiled)
        self.assertEqual(
            test_node.test_metadata.kwargs["color"], "env_var('ENV_VAR_COLOR')"
        )
...
class ProjectEnvVarTest(BasePPTest):
    ...
    def project_config(self):
        ...
        return {
            ...
            "models": {"+meta": {"meta_name": "{{ env_var('ENV_VAR_NAME') }}"}},
        }
    ...
    def test_postgres_project_env_vars(self):
        ...
        self.assertNotEqual(
            env_vars_hash_checksum, manifest.state_check.project_env_vars_hash.checksum
        )
