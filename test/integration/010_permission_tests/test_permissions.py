...
class TestPermissions(DBTIntegrationTest):
    ...
    def test_postgres_create_schema_permissions(self):
        # now it should work!
        self.run_sql(
            "grant create on database {} to noaccess".format(self.default_database)
        )
        self.run_sql(
            'grant usage, create on schema "{}" to noaccess'.format(
                self.unique_schema()
            )
        )
        self.run_sql(
            'grant select on all tables in schema "{}" to noaccess'.format(
                self.unique_schema()
            )
        )
